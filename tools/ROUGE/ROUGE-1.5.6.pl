...
# Date:        07/28/2017,05/26/2005,05/19/2005,04/26/2005,04/03/2005,10/28/2004,10/25/2004,10/21/2004
# Author:      Chin-Yew Lin
...
# Revision Note:
#              1.5.6
#              (1) Added support for Bulgarian Cyrillic by Deem Hristov.
#              1.5.5
...
sub readTextOld {
  ...
  if($type=~/^SEE$/oi) {
	...
	$text=$3;
	$text=~tr/A-ZАБВГДЕЖЗИЙЍКЛМНОПРСТУФХЦЧШЩЪЬЮЯ/a-zабвгдежзийѝклмнопрстуфхцчшщъьюя/;
  ...
  if(defined($$tokenizedText)) {
    $$tokenizedText=~s/\-/ \- /g;
    $$tokenizedText=~s/[^A-Za-zАБВГДЕЖЗИЙЍКЛМНОПРСТУФХЦЧШЩЪЬЮЯабвгдежзийѝклмнопрстуфхцчшщъьюя0-9\-]/ /g;
    $$tokenizedText=~s/^\s+//;
    $$tokenizedText=~s/\s+$//;
    $$tokenizedText=~s/\s+/ /g;
  }
...
sub readText_LCS {
  ...
  if(defined(@{$tokenizedText}>0)) {
    for($t=0;$t<@{$tokenizedText};$t++) {
      $tokenizedText->[$t]=~s/\-/ \- /g;
      $tokenizedText->[$t]=~s/[^A-Za-zАБВГДЕЖЗИЙЍКЛМНОПРСТУФХЦЧШЩЪЬЮЯабвгдежзийѝклмнопрстуфхцчшщъьюя0-9\-]/ /g;
      $tokenizedText->[$t]=~s/^\s+//;
      $tokenizedText->[$t]=~s/\s+$//;
...
sub MorphStem {
  ...
  $ltoken=$token;
  $ltoken=~tr/A-ZАБВГДЕЖЗИЙЍКЛМНОПРСТУФХЦЧШЩЪЬЮЯ/a-zабвгдежзийѝклмнопрстуфхцчшщъьюя/;
...
sub createNGram {
  ...
  for($i=0;$i<=$#mx_tokens;$i++) {
    unless(exists($stopwords{$mx_tokens[$i]})) {
      ...
      if($mx_tokens[$i]=~/^[a-zабвгдежзийѝклмнопрстуфхцчшщъьюя0-9\$]/o) {
	if(defined($opt_m)) {
	  # use stemmer
...
sub createBE {
  ...
  for($i=0;$i<=$#{$BEList};$i++) {
    ...
    my ($be,$stemH,$stemM);
    $be=$BEList->[$i];
    $be=~tr/A-ZАБВГДЕЖЗИЙЍКЛМНОПРСТУФХЦЧШЩЪЬЮЯ/a-zабвгдежзийѝклмнопрстуфхцчшщъьюя/;
    @fds=split(/\|/,$be);
    if(@fds!=3) {
...
sub MorphStemMulti {
  ...
  foreach $t (@tokens) {
    if($t=~/[A-Za-zАБВГДЕЖЗИЙЍКЛМНОПРСТУФХЦЧШЩЪЬЮЯабвгдежзийѝклмнопрстуфхцчшщъьюя0-9]/o&&
